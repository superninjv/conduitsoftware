    ...
    <header className="fixed top-0 left-0 right-0 z-50 nav-blur border-b border-[var(--border-color)]">
      <nav className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
        ...
        <Link href="/" className="flex items-center gap-2.5 group">
          <ConduitLogo className="w-8 h-8 transition-transform group-hover:scale-105" />
        ...
        <div className="hidden md:flex items-center gap-1">
          {siteConfig.nav.map((item) => (
            <Link
              key={item.href}
              href={item.href}
              className="px-4 py-2 text-sm font-medium text-[var(--text-secondary)] hover:text-[var(--text-primary)] rounded-lg hover:bg-[var(--border-color)] transition-all duration-200"
            >
              {item.label}
            </Link>
          ))}
          ...
          <div className="px-6 py-4 flex flex-col gap-1">
            {siteConfig.nav.map((item) => (
              <Link
                ...
                className="px-4 py-3 text-sm font-medium text-[var(--text-secondary)] hover:text-[var(--text-primary)] rounded-lg hover:bg-[var(--border-color)] transition-all"
              >
                {item.label}
              </Link>
            ))}
